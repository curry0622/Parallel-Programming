...
int getFirstBiggerThanTargetIndex(float* arr, int size, float target) {
    int low = 0, high = size;
    while(low != high) {
        int mid = (low + high) / 2;
        if(arr[mid] <= target)
...
void mergeData(float* myDataBuf, int mySize, float* recvDataBuf, int recvSize, float* funcBuf, bool fromMin) {
    if(fromMin) {
        int i = 0, j = 0;
